...
class Cell:
    ...
    @staticmethod
    def make_order(self, quan2):
        whole = self.quantity // quan2
        remains = self.quantity % quan2
        gen_1 = ['*' for num in range(quan2)]
        gen_2 = ["".join(gen_1) for num in range(whole)]
        gen_2.append("".join('*' for num in range(remains)))
        return gen_2
...
print(*my_cell.make_order(my_cell2, 5), sep='\n')